...
describe('normalizeReservationResponse', () => {
  ...
  describe('Field merging', () => {
    ...
    it('should have both id and reservation_id for success screen compatibility', () => {
      const response = {
        reservation_id: '95492c92-test',
        status: 'confirmed'
      };
      ...
      // Both names should be present for frontend compatibility
      expect(result.id).toBeDefined();
      expect(result.reservation_id).toBeDefined();
      expect(result.id).toBe(result.reservation_id);
    });
